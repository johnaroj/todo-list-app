...
export default defineComponent({
  setup() {
    ...
    const toggleTodo = async (todo:Todo)=>{
      const result = await axios.put('http://localhost:8080/todos/' + todo.id, {
    ...
    const removeTodo = async(todo: Todo)=>{
        const result =await axios.delete('https://0je8g9zvze.execute-api.eu-west-1.amazonaws.com/test/todos/'+todo.id);
